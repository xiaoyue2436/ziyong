/**
 ...
 变量：两个 hqAPP1：Authorization hqAid1：aid
 定时一天一次
 cron: 7 9,18 * * *
 ...
 */
 ...
 let hqAPP1 = ($.isNode() ? process.env.hqAPP1 : $.getdata("hqAPP11")) || "";
 let hqAPP1Arr = [];
 let hqAid1 = ($.isNode() ? process.env.hqAid1 : $.getdata("hqAid11")) || ""
 let hqAid1Arr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
     if (typeof $request !== "undefined") {
         ...
         if (!(await Envs()))
             ...
             for (let index = 0; index < hqAPP1Arr.length; index++) {


                 let num = index + 1
                 log(`\n========= 开始【第 ${num} 个账号】=========\n`)

                 hqAPP1 = hqAPP1Arr[index];
                 hqAid1 = hqAid1Arr[index];
...
function doSignin(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {"Authorization":`${hqAPP1}`,"platform":"2","aid":`${hqAid1}`,"version":"3.18.0","Content-Type":"application/json","Host":"hqapp.faw.cn","Connection":"Keep-Alive","Accept-Encoding":"gzip","User-Agent":"okhttp/3.11.0"},
            body: `{"scoreType":"2"}`,
        }
...
async function GetRewrite() {
    if ($request.url.indexOf("v2/getTaskList") > -1) {
        ...
        if (hqAPP1) {
            if (hqAPP1.indexOf(ck1) == -1) {
                hqAPP1 = hqAPP1 + "@" + ck1;
                $.setdata(hqAPP1, "hqAPP1");
                let List = hqAPP1.split("@");
                $.msg(
        ...
        if (hqAid1) {
            if (hqAid1.indexOf(ck2) == -1) {
                hqAid1 = hqAid1 + "@" + ck2;
                $.setdata(hqAid1, "hqAid1");
                let List = hqAid1.split("@");
                $.msg(
 ...
 async function Envs() {
    if (hqAid1) {
        if (hqAid1.indexOf("@") != -1) {
            hqAid1.split("@").forEach((item) => {
                hqAid1Arr.push(item);
            });
        } else if (hqAid1.indexOf("\n") != -1) {
            hqAid1.split("\n").forEach((item) => {
                hqAid1Arr.push(item);
            });
        } else {
            hqAid1Arr.push(hqAid1);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAid1`)
    }
    if (hqAPP1) {
        if (hqAPP1.indexOf("@") != -1) {
            hqAPP1.split("@").forEach((item) => {
                hqAPP1Arr.push(item);
            });
        } else if (hqAPP1.indexOf("\n") != -1) {
            hqAPP1.split("\n").forEach((item) => {
                hqAPP1Arr.push(item);
            });
        } else {
            hqAPP1Arr.push(hqAPP1);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAPP1`)
        return;
