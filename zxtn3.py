...
手机号填入环境变量zxtn，换行分割
每天一次即可
# cron "22 */12 * * *" script-path=xxx.py,tag=匹配cron用
# const $ = new Env('掌心潼南3');
作者：cason
...
def news(token):
    ...
    try:
        with open('掌心潼南3.json', 'r', encoding='utf-8') as f1:
    ...
    if res.json().get('code') == 0:
        ...
        for num, news in enumerate(news_list):
            ...
            if num == 9:
                ...
                with open('掌心潼南3.json', 'w', encoding='utf-8') as f:
...
def main():
    content = ''
    phone_list = os.getenv('zxtn3').split('\n')
