...
# 在gqft_cookie4.txt里面添加refreshToken
# 有可能点赞跑不完，还有兑换功能，自己琢磨，只能跑一个号，你可以建立多个gqft.py  gqft_cookie4.txt来跑
...
# 更新gqft_cookie4里面的数据
def set_Au(AU):
    with open(file='gqft_cookie4.txt', mode='w', encoding='utf-8') as f:
        js_data = f.write(AU)
        f.close()
# 获取gqft_cookie4里面的数据
def get_Au():
    with open(file='gqft_cookie4.txt', mode='r', encoding='utf-8') as f:
        js_data = f.read()
        f.close()
    return js_data
# 解密sig
