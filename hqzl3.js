/**
 ...
 变量：两个 hqAPP3：Authorization hqAid3：aid
 定时一天一次
 cron: 11 9,18 * * *
 ...
 */
 ...
 let hqAPP3 = ($.isNode() ? process.env.hqAPP3 : $.getdata("hqAPP31")) || "";
 let hqAPP3Arr = [];
 let hqAid3 = ($.isNode() ? process.env.hqAid3 : $.getdata("hqAid31")) || ""
 let hqAid3Arr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
     if (typeof $request !== "undefined") {
         ...
         if (!(await Envs()))
             ...
             for (let index = 0; index < hqAPP3Arr.length; index++) {


                 let num = index + 1
                 log(`\n========= 开始【第 ${num} 个账号】=========\n`)

                 hqAPP3 = hqAPP3Arr[index];
                 hqAid3 = hqAid3Arr[index];
...
function doSignin(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {"Authorization":`${hqAPP3}`,"platform":"2","aid":`${hqAid3}`,"version":"3.18.0","Content-Type":"application/json","Host":"hqapp.faw.cn","Connection":"Keep-Alive","Accept-Encoding":"gzip","User-Agent":"okhttp/3.11.0"},
            body: `{"scoreType":"2"}`,
        }
...
async function GetRewrite() {
    if ($request.url.indexOf("v2/getTaskList") > -1) {
        ...
        if (hqAPP3) {
            if (hqAPP3.indexOf(ck1) == -1) {
                hqAPP3 = hqAPP3 + "@" + ck1;
                $.setdata(hqAPP3, "hqAPP3");
                let List = hqAPP3.split("@");
                $.msg(
        ...
        if (hqAid3) {
            if (hqAid3.indexOf(ck2) == -1) {
                hqAid3 = hqAid3 + "@" + ck2;
                $.setdata(hqAid3, "hqAid3");
                let List = hqAid3.split("@");
                $.msg(
 ...
 async function Envs() {
    if (hqAid3) {
        if (hqAid3.indexOf("@") != -1) {
            hqAid3.split("@").forEach((item) => {
                hqAid3Arr.push(item);
            });
        } else if (hqAid3.indexOf("\n") != -1) {
            hqAid3.split("\n").forEach((item) => {
                hqAid3Arr.push(item);
            });
        } else {
            hqAid3Arr.push(hqAid3);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAid3`)
    }
    if (hqAPP3) {
        if (hqAPP3.indexOf("@") != -1) {
            hqAPP3.split("@").forEach((item) => {
                hqAPP3Arr.push(item);
            });
        } else if (hqAPP3.indexOf("\n") != -1) {
            hqAPP3.split("\n").forEach((item) => {
                hqAPP3Arr.push(item);
            });
        } else {
            hqAPP3Arr.push(hqAPP3);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAPP3`)
        return;
