/**
 ...
 变量：两个 hqAPP2：Authorization hqAid2：aid
 定时一天一次
 cron: 9 9,18 * * *
 ...
 */
 ...
 let hqAPP2 = ($.isNode() ? process.env.hqAPP2 : $.getdata("hqAPP21")) || "";
 let hqAPP2Arr = [];
 let hqAid2 = ($.isNode() ? process.env.hqAid2 : $.getdata("hqAid21")) || ""
 let hqAid2Arr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
     if (typeof $request !== "undefined") {
         ...
         if (!(await Envs()))
             ...
             for (let index = 0; index < hqAPP2Arr.length; index++) {


                 let num = index + 1
                 log(`\n========= 开始【第 ${num} 个账号】=========\n`)

                 hqAPP2 = hqAPP2Arr[index];
                 hqAid2 = hqAid2Arr[index];
...
function doSignin(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {"Authorization":`${hqAPP2}`,"platform":"2","aid":`${hqAid2}`,"version":"3.18.0","Content-Type":"application/json","Host":"hqapp.faw.cn","Connection":"Keep-Alive","Accept-Encoding":"gzip","User-Agent":"okhttp/3.11.0"},
            body: `{"scoreType":"2"}`,
        }
...
async function GetRewrite() {
    if ($request.url.indexOf("v2/getTaskList") > -1) {
        ...
        if (hqAPP2) {
            if (hqAPP2.indexOf(ck1) == -1) {
                hqAPP2 = hqAPP2 + "@" + ck1;
                $.setdata(hqAPP2, "hqAPP2");
                let List = hqAPP2.split("@");
                $.msg(
        ...
        if (hqAid2) {
            if (hqAid2.indexOf(ck2) == -1) {
                hqAid2 = hqAid2 + "@" + ck2;
                $.setdata(hqAid2, "hqAid2");
                let List = hqAid2.split("@");
                $.msg(
 ...
 async function Envs() {
    if (hqAid2) {
        if (hqAid2.indexOf("@") != -1) {
            hqAid2.split("@").forEach((item) => {
                hqAid2Arr.push(item);
            });
        } else if (hqAid2.indexOf("\n") != -1) {
            hqAid2.split("\n").forEach((item) => {
                hqAid2Arr.push(item);
            });
        } else {
            hqAid2Arr.push(hqAid2);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAid2`)
    }
    if (hqAPP2) {
        if (hqAPP2.indexOf("@") != -1) {
            hqAPP2.split("@").forEach((item) => {
                hqAPP2Arr.push(item);
            });
        } else if (hqAPP2.indexOf("\n") != -1) {
            hqAPP2.split("\n").forEach((item) => {
                hqAPP2Arr.push(item);
            });
        } else {
            hqAPP2Arr.push(hqAPP2);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAPP2`)
        return;
