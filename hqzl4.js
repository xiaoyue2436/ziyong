/**
 ...
 变量：两个 hqAPP4：Authorization hqAid4：aid
 定时一天一次
 cron: 13 9,18 * * *
 ...
 */
 ...
 let hqAPP4 = ($.isNode() ? process.env.hqAPP4 : $.getdata("hqAPP41")) || "";
 let hqAPP4Arr = [];
 let hqAid4 = ($.isNode() ? process.env.hqAid4 : $.getdata("hqAid41")) || ""
 let hqAid4Arr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
     if (typeof $request !== "undefined") {
         ...
         if (!(await Envs()))
             ...
             for (let index = 0; index < hqAPP4Arr.length; index++) {


                 let num = index + 1
                 log(`\n========= 开始【第 ${num} 个账号】=========\n`)

                 hqAPP4 = hqAPP4Arr[index];
                 hqAid4 = hqAid4Arr[index];
...
function doSignin(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {"Authorization":`${hqAPP4}`,"platform":"2","aid":`${hqAid4}`,"version":"3.18.0","Content-Type":"application/json","Host":"hqapp.faw.cn","Connection":"Keep-Alive","Accept-Encoding":"gzip","User-Agent":"okhttp/3.11.0"},
            body: `{"scoreType":"2"}`,
        }
...
async function GetRewrite() {
    if ($request.url.indexOf("v2/getTaskList") > -1) {
        ...
        if (hqAPP4) {
            if (hqAPP4.indexOf(ck1) == -1) {
                hqAPP4 = hqAPP4 + "@" + ck1;
                $.setdata(hqAPP4, "hqAPP4");
                let List = hqAPP4.split("@");
                $.msg(
        ...
        if (hqAid4) {
            if (hqAid4.indexOf(ck2) == -1) {
                hqAid4 = hqAid4 + "@" + ck2;
                $.setdata(hqAid4, "hqAid4");
                let List = hqAid4.split("@");
                $.msg(
 ...
 async function Envs() {
    if (hqAid4) {
        if (hqAid4.indexOf("@") != -1) {
            hqAid4.split("@").forEach((item) => {
                hqAid4Arr.push(item);
            });
        } else if (hqAid4.indexOf("\n") != -1) {
            hqAid4.split("\n").forEach((item) => {
                hqAid4Arr.push(item);
            });
        } else {
            hqAid4Arr.push(hqAid4);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAid4`)
    }
    if (hqAPP4) {
        if (hqAPP4.indexOf("@") != -1) {
            hqAPP4.split("@").forEach((item) => {
                hqAPP4Arr.push(item);
            });
        } else if (hqAPP4.indexOf("\n") != -1) {
            hqAPP4.split("\n").forEach((item) => {
                hqAPP4Arr.push(item);
            });
        } else {
            hqAPP4Arr.push(hqAPP4);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAPP4`)
        return;
