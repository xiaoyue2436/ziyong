 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export yshy_data=' authorization '   ,多账号用 换行 或 @ 分割
 * 抓hy.51pt.top下的authorization填入变量
 */

 const { log } = require("console");

 const $ = new Env("仰韶会员俱乐部 小程序");
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.yshy_data : $.getdata('yshy_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'hy.51pt.top';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "yshy_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/app/ys/signIn/save`,
			 headers: {
				 "Host": host,
				 'Content-Length': '25',
